...
type CardContentProps = { element: DataType; category: DataCategoryType };
  ...
  const renderTitle = () => {
    return (
      <Stack>
        <Typography
          level="title-lg"
          sx={{ "&:hover": { color: getCategoryColor(category) } }}
        >
            ...
            {element.dateEnd && (
              <>
                ...
                {typeof element.dateEnd === "string"
                  ? element.dateEnd
                  : `${getDateMonth(element.dateEnd, language)} ${getDateYear(
                      element.dateEnd
                    )}`}
              </>
            )}
  ...
  return (
    <ListItem key={element.id} sx={{ p: 0 }}>
      ...
      >
        ...
        >
          {renderAspectRatio()}
          <Box sx={{ display: { xs: "block", lg: "none" }, flex: 1 }}>
            {renderTitle()}
          </Box>
        </Stack>
